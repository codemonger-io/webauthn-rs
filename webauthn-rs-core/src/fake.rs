...
{
    // hmac key (clone it before using it)
    ...
    D: FakeCredentialIDDistribution,
{
    ...
    pub fn new(hmac_key: &[u8]) -> Result<Self, WebauthnError> {
        let hmac_key = Hmac::<Sha256>::new_from_slice(hmac_key)
            .map_err(|e| {
                error!(?e, "creating an HMAC key");
                WebauthnError::TransientError("creating an HMAC key")
            })?;
